...
export default function CategoryFilter({ categories, todosCountMap, totalTodos }: CategoryFilterProps) {
  ...
  return (
    <div class="flex items-center gap-3 overflow-x-auto py-2 px-1 scrollbar-hide no-scrollbar">
      ...
      >
        Hepsi
        <span class={`px-2 py-0.5 rounded-lg text-[10px] ${activeId === null ? 'bg-white/20 dark:bg-black/10' : 'bg-gray-100 dark:bg-gray-700'}`}>
          {totalTodos}
        </span>
      </button>
      ...
      {categories.map((category) => {
        ...
        return (
          <button
            ...
            style={isActive ? { backgroundColor: category.color, boxShadow: `0 10px 15px -3px ${category.color}33` } : {}}
          >
            <span class="w-2.5 h-2.5 rounded-full ring-2 ring-white/20" style={{ backgroundColor: isActive ? 'white' : category.color }}></span>
            {category.name}
            <span class={`px-2 py-0.5 rounded-lg text-[10px] ${isActive ? 'bg-white/20' : 'bg-gray-100 dark:bg-gray-700'}`}>
