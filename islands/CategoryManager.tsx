...
export default function CategoryManager(
  { initialCategories, todosCountMap, csrfToken }: CategoryManagerProps,
) {
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        "Bu kategoriyi silmek istediğinize emin misiniz? Görevler silinmeyecektir.",
      )
    ) return;
        ...
        {categories.length === 0
          ...
          : (
            categories.map((c) => (
              <div
                key={c.id}
                class="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 hover:shadow-md transition-all duration-300 animate-scale-in"
              >
                {editingId === c.id
                  ? (
                    <div class="space-y-4">
                      <div class="flex items-center gap-4">
                        <input
                          ...
                          value={editColor}
                          onInput={(e) =>
                            setEditColor((e.target as HTMLInputElement).value)}
                      ...
                      <div class="flex justify-end gap-2">
                        <button
                          ...
                          class="px-4 py-2 text-sm font-bold text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl transition-all"
                        >
                          İptal
                        </button>
                        <button
                          ...
                          class="px-4 py-2 text-sm font-bold text-white bg-green-600 hover:bg-green-700 rounded-xl transition-all shadow-lg shadow-green-500/20"
                        >
                          Kaydet
                        </button>
                      </div>
                  ...
                  : (
                    <div class="flex items-center justify-between">
                      <div class="flex items-center gap-4">
                        <div
                          class="w-12 h-12 rounded-2xl flex items-center justify-center text-white font-bold shadow-lg"
                          style={{
                            backgroundColor: c.color,
                            boxShadow: `0 8px 15px -3px ${c.color}44`,
                          }}
                        >
                      ...
                      <div class="flex gap-1">
                        <button
                          type="button"
                          onClick={() => startEdit(c)}
                          class="p-2 text-gray-400 hover:text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-900/20 rounded-xl transition-all"
                          title="Düzenle"
                        >
                          <svg
                            class="w-5 h-5"
