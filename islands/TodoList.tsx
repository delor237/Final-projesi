...
import { Category, Todo } from "../utils/db.ts";
...
export default function TodoList(
  { initialTodos, categories, csrfToken }: TodoListProps,
) {
  ...
  const toggleTodo = async (id: string) => {
    ...
    todosStore.value = todosStore.value.map((t) =>
    ...
    return (
      <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-gray-100 dark:border-gray-800 p-12 text-center animate-scale-in">
        ...
        <p class="text-sm text-gray-400 mt-1">
          Sol taraftaki formdan yeni bir görev ekleyerek başlayın.
        </p>
      </div>
    );
  }
          ...
          : (
            filteredTodos.map((todo) => {
              ...
              return (
                <li
                  key={todo.id}
                  class="group p-5 hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-all duration-300"
                >
                  <div class="flex items-start gap-4">
                    <div class="flex-shrink-0 pt-1">
                      <div class="relative">
                        ...
                        />
                        <svg
                          class="absolute top-1 left-1 h-4 w-4 text-white pointer-events-none opacity-0 peer-checked:opacity-100 transition-opacity"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                          stroke-width="4"
                    ...
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-3 flex-wrap mb-1">
                        <p
                          class={`text-lg font-semibold tracking-tight transition-all ${
                            todo.completed
                        ...
                        {category && (
                          <button
                            ...
                            onClick={(e) => {
                              e.preventDefault();
                              activeCategoryStore.value = category.id;
                            }}
