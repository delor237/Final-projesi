...
export default function Home({ data, state }: PageProps<Data, State>) {
    ...
    return (
      <div class="flex flex-col items-center justify-center py-20 animate-fade-in-up">
        ...
        <h1 class="text-5xl font-display font-extrabold text-gray-900 dark:text-white mb-6 text-center tracking-tight">
          Modern Görev <span class="bg-clip-text text-transparent bg-gradient-to-r from-brand-600 to-brand-400">Yönetimi</span>
        </h1>
        <p class="text-xl text-gray-500 dark:text-gray-400 max-w-2xl text-center mb-10 leading-relaxed">
          İşlerinizi organize edin, kategorize edin ve verimliliğinizi artırın. Deno Fresh teknolojisiyle yıldırım hızında deneyim.
        </p>
  ...
  return (
    <div class="max-w-6xl mx-auto space-y-10 animate-fade-in-up">
      <header class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p class="text-lg text-gray-500 dark:text-gray-400 mt-2 font-medium">
            Bugün yapacak {data.todos.filter(t => !t.completed).length} göreviniz var.
          </p>
        </div>
      ...
      <div class="relative">
        <div class="absolute inset-0 bg-gradient-to-r from-brand-500/5 to-transparent rounded-3xl -z-10"></div>
        <CategoryFilter categories={data.categories} todosCountMap={data.todosCountMap} totalTodos={data.todos.length} />
      </div>
      
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div class="lg:col-span-4 space-y-6">
          <div class="sticky top-24">
            <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-4 px-1">Yeni Görev Ekle</h2>
            <TodoForm categories={data.categories} csrfToken={state.csrfToken} />
          </div>
        </div>
        <div class="lg:col-span-8">
          <div class="flex items-center justify-between mb-4 px-1">
            <h2 class="text-xl font-bold text-gray-900 dark:text-white">Görev Listesi</h2>
            <span class="text-xs font-bold text-gray-400 uppercase tracking-widest">
              {data.todos.length} Toplam
            </span>
          </div>
          <TodoList initialTodos={data.todos} categories={data.categories} csrfToken={state.csrfToken} />
        </div>
      </div>
    </div>
  );
